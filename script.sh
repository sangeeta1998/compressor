...
measure_execution_time() {
    ...
    echo "Starting container..."
    {
        if [ -z "$runtime" ]; then
            ...
            /usr/bin/time -v docker run --rm --mount type=bind,source="$(pwd)",target=/app $image
        else
            # For Wasm containers with specific runtime
            echo "Running Wasm container with runtime: $runtime"
            /usr/bin/time -v docker run --runtime=$runtime --platform=$platform --rm --mount type=bind,source="$(pwd)",target=/app $image
        fi
...
# Define image names
rust_native_image="sangeetakakati/rust-compressor-native"
rust_wasm_image="sangeetakakati/rust-compressor-wasm"
...
for image in "$rust_native_image" "$rust_wasm_image"; do
